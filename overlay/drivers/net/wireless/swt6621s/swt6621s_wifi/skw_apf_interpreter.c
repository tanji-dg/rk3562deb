...
extern void APF_TRACE_HOOK(uint32_t pc, const uint32_t *regs,
			   const uint8_t *program, uint32_t program_len,
			   const uint8_t *packet, uint32_t packet_len,
			   const uint32_t *memory, uint32_t ram_len);
#else
#define APF_TRACE_HOOK(pc, regs, program, program_len, packet, packet_len,     \
		       memory, memory_len)                                     \
	do { /* nop*/                                                          \
	} while (0)
#endif
...
#define IN_DATA_BOUNDS(p, size)                                                \
	(ENFORCE_UNSIGNED(p) && ENFORCE_UNSIGNED(size) &&                      \
	 (p) + (size) <= ram_len && (p) >= program_len &&                      \
...
#define ASSERT_FORWARD_IN_PROGRAM(p)                                           \
	ASSERT_RETURN(IN_PROGRAM_BOUNDS(p) && (p) >= pc)
	// Memory slot values.
	...
	uint32_t tmp;
	int32_t shift_val;
	uint32_t cmp_imm_len;
	uint32_t div_operand;
	uint32_t offs;
	uint32_t size;
	uint32_t val;
	uint32_t last_packet_offs;
	uint32_t imm_len;
	uint32_t load_size;
	uint32_t end_offs;
	...
	instructions_remaining = program_len;

	do {
		APF_TRACE_HOOK(pc, registers, program, program_len, packet,
			       packet_len, memory, ram_len);
...
#define OTHER_REG (registers[reg_num ^ 1])
		...
		case JNEBS_OPCODE: {
			...
			case JNEBS_OPCODE: {
				...
				last_packet_offs = REG + cmp_imm - 1;
		...
		case MUL_OPCODE:
			...
			break;
		case DIV_OPCODE: {
			div_operand = reg_num ? registers[1] : imm;
...
// instead just enforce that imm is unsigned (so it's always greater or equal to 0).
		...
		case LDDW_OPCODE: {
			...
			if (offs & 0x80000000) {
				offs = ram_len +
				       offs; // unsigned overflow intended
			}
