...
static int dwc3_setup_role_switch(struct dwc3 *dwc)
{
	...
	/*
	 * No Type-C port manager drives this switch on the RK3562 tablet, so
	 * expose /sys/class/usb_role/<sw>/role and let user space pick the role
	 * (default stays "device" via role-switch-default-mode).
	 */
