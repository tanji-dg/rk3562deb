...
apt-get install -y sudo curl wget nano vim openssh-server network-manager wpasupplicant iw wireless-tools \
    ...
    python3 \
    iproute2 iputils-ping dnsutils locales tzdata xfce4-power-manager \
...
if [ -d "${MODULES_DIR}" ]; then
    ...
    cp -a "${MODULES_DIR}"/* "${ROOTFS_MNT}/lib/modules/"
else
...
if [ -d "${ROOT_DIR}/overlay/firmware" ]; then
    cp -af "${ROOT_DIR}/overlay/firmware/." "${ROOTFS_MNT}/lib/firmware/" 2>/dev/null || true
fi
