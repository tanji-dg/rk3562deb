...
# Docker's json-file log driver has no rotation by default, so container logs
# grow without bound. A single misbehaving container (the kozekachi_ai browser
# stack runs several) can fill the rootfs and take the system down. Cap every
# container at 3 x 10 MiB.
if [ -f "${ROOT_DIR}/overlay/etc/docker/daemon.json" ]; then
    echo "[*] Installing Docker log rotation defaults..."
    mkdir -p "${ROOTFS_MNT}/etc/docker"
    cp "${ROOT_DIR}/overlay/etc/docker/daemon.json" "${ROOTFS_MNT}/etc/docker/daemon.json"
    chmod 0644 "${ROOTFS_MNT}/etc/docker/daemon.json"
fi
...
mkdir -p "${ROOTFS_MNT}/usr/local/sbin"
cp "${ROOT_DIR}/overlay/rk-powerkey-longpress.py" \
    "${ROOTFS_MNT}/usr/local/sbin/rk-powerkey-longpress.py"
...
# 10a. RK817 charge cap (CV 4.10V) — keeps the battery off a full 4.20V charge
# to extend calendar life. The tablet is mains-powered nearly all the time, so
# topping out around 80-85% SoC costs nothing in practice.
if [ -f "${ROOT_DIR}/overlay/chrg-cap" ] && [ -f "${ROOT_DIR}/overlay/chrg-cap.service" ]; then
    echo "[*] Installing RK817 charge cap service..."
    cp "${ROOT_DIR}/overlay/chrg-cap" "${ROOTFS_MNT}/usr/local/sbin/chrg-cap"
    chmod +x "${ROOTFS_MNT}/usr/local/sbin/chrg-cap"
    cp "${ROOT_DIR}/overlay/chrg-cap.service" "${ROOTFS_MNT}/etc/systemd/system/chrg-cap.service"
    chroot "${ROOTFS_MNT}" systemctl enable chrg-cap.service
fi
